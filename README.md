...
- 在「计费」页维护模型价格(元/1M tokens),模型名支持 `*` 前缀通配(如 `gpt-*`);精确命中优先,其次取最长通配前缀
- 成功的请求按用量折算费用记入 `request_logs.cost`,并从用户余额中同步扣除(日志与扣费同一事务);无价格匹配的模型按 0 元记账
- 余额为用户级,**不足时不拦截请求**(允许为负),只记账;充值/扣减由管理员在「用户」页手工调账,流水在「计费」页可查
